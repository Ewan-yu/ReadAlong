...
function VoiceCard({ voice, onDefault, onDelete, onRename, busy }: { voice: VoiceProfile; onDefault: () => void; onDelete: () => void; onRename: (name: string) => void; busy: boolean }) {
  ...
  return <article className={styles.voiceCard} data-state={voice.status}>
    ...
    {voice.status === "ready" ? <><audio controls preload="none" src={voicePreviewUrl(voice.voice_id)} /><div className={styles.meta}><span>{voice.reference_duration_seconds?.toFixed(1)} 秒参考声</span><span>{voice.clone_mode === "hifi" ? "Hi‑Fi 实际克隆试听" : "实际克隆试听"}</span></div><div className={styles.actions}>{!voice.is_default && <button type="button" disabled={busy} onClick={onDefault}><Star />设为默认</button>}{!voice.is_system && <button type="button" data-danger disabled={busy || voice.is_default} onClick={onDelete}><Trash2 />删除</button>}</div></> : voice.status === "failed" ? <p className={styles.failure}><AlertTriangle />{voice.failure_message ?? "处理失败，请重新创建样本。"}</p> : <p className={styles.processing}><LoaderCircle className={styles.spin} />{voice.progress_message ?? "正在处理声音样本…"}</p>}
...
export function VoiceProfilesPage() {
  ...
  const [uploadedDuration, setUploadedDuration] = useState(12);
  ...
  const upload = useMutation({ mutationFn: () => uploadedFile ? uploadVoiceProfile(uploadedName.trim(), uploadedFile, uploadedDuration) : Promise.reject(new Error("请选择声音文件")), onSuccess: () => { setUploadedFile(undefined); setUploadedName(""); setUploadedDuration(12); refresh(); } });
  ...
  return <section className={styles.page}>
    ...
    <section className={styles.create}><div className={styles.createLead}><Plus /><div><h2>添加声音样本</h2><p>每个样本都先生成固定参考声和实际克隆试听；试听满意后才建议设为默认。</p></div></div><div className={styles.createForms}><form onSubmit={(event) => { event.preventDefault(); if (generatedName.trim() && generatedDescription.trim()) createGenerated.mutate(); }}><h3>系统生成</h3><p>自动生成约 8–12 秒参考声，并以 Hi‑Fi 克隆制作试听。</p><label>样本名称<input value={generatedName} onChange={(event) => setGeneratedName(event.target.value)} placeholder="例如：温暖女老师" /></label><label>声音描述<textarea value={generatedDescription} onChange={(event) => setGeneratedDescription(event.target.value)} placeholder="使用你自己的声音描述" /></label><button disabled={busy || !generatedName.trim() || generatedDescription.trim().length < 3} type="submit"><MicVocal />创建 Hi‑Fi 声音样本</button></form><form onSubmit={(event) => { event.preventDefault(); if (uploadedFile && uploadedName.trim()) upload.mutate(); }}><h3>上传克隆</h3><p>上传 5–30 秒清晰、无音乐、单人 WAV 或 MP3。没有逐字稿时采用稳定的基础克隆。</p><label>样本名称<input value={uploadedName} onChange={(event) => setUploadedName(event.target.value)} placeholder="例如：妈妈的故事声" /></label><label>参考时长 <b>{uploadedDuration} 秒</b><input type="range" min="5" max="30" step="1" value={uploadedDuration} onChange={(event) => setUploadedDuration(Number(event.target.value))} /></label><label>参考音频<input type="file" accept="audio/wav,audio/mpeg,.wav,.mp3" onChange={(event) => setUploadedFile(event.target.files?.[0])} /></label><button disabled={busy || !uploadedFile || !uploadedName.trim()} type="submit"><FileAudio />上传并生成试听</button></form></div>{error && <p className={styles.failure} role="alert"><AlertTriangle />{(error as Error).message}</p>}</section>
  </section>;
}
