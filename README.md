...
截至 2026-08-02，M0–M5.5 的代码闭环已完成；家长端 132 项 pytest、阅读端 250 项 Flutter 测试、`flutter analyze` 和资源包格式门禁均通过。本轮还补齐了绘本删除时永久配音级联清理、资源包体积/结构校验、家长端取消与子进程回收、回滚异常隔离和错误详情脱敏。剩余工作是 Android 14 实体平板与 2–3 本真书的端到端回归、长时模型取消的真实设备测量、性能测量及正式 release 验收。
